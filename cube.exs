defmodule Cube do
  ...
  def draw(vertices, connect) do
    Enum.map(connect, fn pair -> 
      ...
      #20 is focal length, 25 is focal length + some initial offset to stop cube from "coming out of screen"
      [screenY1, screenY2] = Enum.map([{y1, z1}, {y2, z2}], fn yz -> round(elem(:io.rows(), 1)/2 - (elem(yz, 0) * 20)/(25 - elem(yz, 1))*0.6) end)
      [screenX1, screenX2] = Enum.map([{x1, z1}, {x2, z2}], fn xz -> round((elem(xz, 0) * 20)/(25 - elem(xz, 1)) + elem(:io.columns(), 1)/2) end)
